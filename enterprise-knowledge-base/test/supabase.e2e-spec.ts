...
describe('Supabase Connectivity (e2e)', () => {
  ...
  it(
    'should connect to Supabase PostgreSQL and respond to query',
    async () => {
  ...
  it(
    'should check if pgvector extension is available',
    async () => {
      ...
      if (vectorRes.rows.length > 0) {
        console.log(`[Jest E2E] Supabase pgvector version: v${vectorRes.rows[0].extversion}`);
      } else {
        console.warn('[Jest E2E] Supabase pgvector extension is not installed yet.');
      }
    },
    15000,
  );
